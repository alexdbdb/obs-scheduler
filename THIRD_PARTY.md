...
Current Broadcast Scheduler source: **GPL-3.0-or-later**. Earlier source was GPL-2.0-or-later; this release exercises its later-version permission. Copyright 2026 Broadcast Scheduler contributors. Maintained by alexdbdb and contributors.

| Component | License selected | Purpose |
| --- | --- | --- |
| OBS libobs / frontend API | GPL-2.0-or-later | Native integration; supplied by OBS |
| Qt Core, Widgets, Network, Concurrent, Test, WebSockets | GPL-3.0 distribution option (also offered under LGPL/commercial terms) | UI, event loops, networking, Windows Schannel TLS backend and tests |
...
**The linked open-source binary must be distributed under GPLv3**, with OBS's later-version permission and the applicable upstream terms. Qt HTTP Server is GPLv3-only, not LGPL. Do not describe the combined binary as GPLv2-only. See [Qt HTTP Server licensing](https://doc.qt.io/qt-6/qthttpserver-index.html#licenses), [libical license](https://github.com/libical/libical/blob/v3.0.20/LICENSE), [historical GPLv2 text](licenses/GPL-2.0.txt) and [GPLv3](licenses/GPL-3.0.txt).

No proprietary Google SDK is linked: Google integration uses documented HTTPS APIs. Each user configures their own Desktop OAuth client at runtime. No Google credentials are build inputs or bundled in current releases. User access/refresh tokens and Odoo keys must never be published. Historical 0.1.0 binaries require separate review before redistribution; see SECURITY.md.

Before distributing binaries, include the corresponding source (this repository, exact dependency sources and build scripts), copyright notices and applicable license texts. Do not ship unrelated OBS SDK DLLs or replace OBS's Qt Core/Gui/Widgets DLLs. The Windows package keeps the two extra Qt modules beside its own DLL and the matching Schannel backend under its own data/qt/tls directory. Build dependencies are cached locally in `.deps`; that ignored directory is not part of the public repository. See [source distribution](docs/source-distribution.md) for upstream sources and binary-release requirements. `scripts/build-windows.ps1` verifies official OBS dependency SHA-256 hashes from the pinned OBS buildspec. Libical and Qt source tags are pinned; SQLite's source archive version is pinned.
